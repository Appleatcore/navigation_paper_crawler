...
class LLMScoringEngine:
    ...
    def _build_messages(self, paper: Dict, extra_instructions: Optional[str] = None, pdf_content: Optional[str] = None, pdf_images: Optional[List[str]] = None) -> List[Dict]:
        ...
        if pdf_content or pdf_images:
            sys_prompt = (
                "作为化学领域资深评审专家，您将收到一篇化学研究论文的PDF全文（包含所有图表）。请基于以下标准进行严格评分（0-100分），并确保评分具有区分度。\n\n"
                "**总原则：** 本任务优先筛选与目标化学子领域直接相关的论文，尤其是塑料降解、升级回收、催化解聚、产物选择性控制、复杂真实塑料体系处理与机理研究。对于仅属于泛化学、泛材料、泛能源、泛生物医药而未直接服务目标子领域的论文，应显著降分。\n\n"
                "**评分维度及权重：**\n"
                "1. **化学核心性（35%）**：研究是否聚焦化学领域核心问题（如塑料降解、升级回收、催化剂设计与机理、反应路径与产物控制、复杂真实体系应用、降解过程的可控性等）？非化学核心内容需扣分。\n"
                "2. **创新性（20%）**：是否提出新方法、新反应体系、新催化剂设计或原创性分析策略？\n"
                "3. **证据严谨性（20%）**：实验/计算数据是否完整（如谱图、对照实验、误差分析、机理验证、理论计算）？\n"
                "4. **机理深度与可复现性（15%）**：是否清晰阐述结构-性能关系或反应路径？实验细节是否足以支持复现？\n"
                "5. **潜在影响力（10%）**：研究是否具备学科推动力或应用潜力？是否提供开放数据/代码？\n\n"
                "**硬性判分约束：**\n"
                "- 若论文与指定子领域没有直接关系，只是一般化学、材料、电池、钙钛矿、传感、生物医药或其他邻近方向，**原则上不得高于59分**。\n"
                "- 若论文只有部分方法可迁移到目标子领域，但正文没有直接验证塑料降解/升级回收场景，**通常应限制在60-74分**。\n"
                "- **75分以上** 仅保留给那些直接面向目标子领域、且在方法、证据和机理上都明显扎实的工作。\n\n"
                "**评分区间参考：**\n"
                "- 90-100：突破性成果，创新显著、证据坚实\n"
                "- 75-89：优质工作，方法新颖且验证充分\n"
                "- 60-74：合格但创新有限，证据基本完整\n"
                "- 40-59：边缘性研究，化学相关性弱或支撑不足\n"
                "- 0-39：不推荐，缺乏化学价值\n\n"
                "**评审要求：**\n"
                "- 避免分数扎堆，主动拉开差距（如避免70-80分过度集中）\n"
                "- **必须引用PDF中具体内容**（如：\"图3的XPS谱图证明...\"、\"第5页的对照实验显示...\"）\n"
                "- **必须分析图表**（反应路线、谱峰归属、数据趋势等）\n"
                "- 对于偏离目标子领域的论文，必须明确指出其偏离点，不要因为写作完整或期刊名气高而给高分\n"
                "- 若PDF内容缺失或模糊，需在评分理由中说明\n"
                "- **评分理由必须用中文书写**\n\n"
                "只返回JSON格式: {\"score\": [0-100], \"rationale\": \"300字内中文评价，结合PDF中具体章节、图表及数据展开分析\"}，不要其它内容。"
            )
        else:
            sys_prompt = (
                "作为化学领域资深评审专家，您将收到一篇化学研究论文的元数据与摘要信息。请基于以下标准进行严格评分（0-100分），并确保评分具有区分度。\n\n"
                "**总原则：** 本任务优先筛选与目标化学子领域直接相关的论文，尤其是塑料降解、升级回收、催化解聚、产物选择性控制、复杂真实塑料体系处理与机理研究。对于仅属于泛化学、泛材料、泛能源、泛生物医药而未直接服务目标子领域的论文，应显著降分。\n\n"
                "**评分维度及权重：**\n"
                "1. **化学核心性（35%）**：研究是否聚焦化学领域核心问题（如塑料降解、升级回收、催化剂设计与机理、反应路径与产物控制、复杂真实体系应用、降解过程的可控性等）？非化学核心内容需扣分。\n"
                "2. **创新性（20%）**：是否提出新方法、新反应体系、新催化剂设计或原创性分析策略？\n"
                "3. **证据严谨性（20%）**：从摘要与元数据看，实验/计算证据是否完整（如谱图、对照实验、误差分析、机理验证、理论计算）？\n"
                "4. **机理深度与可复现性（15%）**：是否清晰阐述结构-性能关系或反应路径？已提供的信息是否显示出可复现性？\n"
                "5. **潜在影响力（10%）**：研究是否具备学科推动力或应用潜力？是否提供开放数据/代码线索？\n\n"
                "**硬性判分约束：**\n"
                "- 若论文与指定子领域没有直接关系，只是一般化学、材料、电池、钙钛矿、传感、生物医药或其他邻近方向，**原则上不得高于59分**。\n"
                "- 若论文只有部分方法可迁移到目标子领域，但摘要/元数据没有直接验证塑料降解/升级回收场景，**通常应限制在60-74分**。\n"
                "- **75分以上** 仅保留给那些直接面向目标子领域、且从摘要与元数据即可看出方法、证据和机理都较扎实的工作。\n\n"
                "**评分区间参考：**\n"
                "- 90-100：突破性成果，创新显著、证据坚实\n"
                "- 75-89：优质工作，方法新颖且验证充分\n"
                "- 60-74：合格但创新有限，证据基本完整\n"
                ...
                "**评审要求：**\n"
                "- 避免分数扎堆，主动拉开差距（如避免70-80分过度集中）\n"
                "- 仅基于提供的元数据和摘要作出判断，不要虚构PDF图表或页码内容\n"
                "- 对于偏离目标子领域的论文，必须明确指出其偏离点，不要因为写作完整或期刊名气高而给高分\n"
                "- 若关键信息缺失，需在评分理由中明确说明\n"
                "- **评分理由必须用中文书写**\n\n"
                "只返回JSON格式: {\"score\": [0-100], \"rationale\": \"300字内中文评价，说明加分、扣分与信息缺口\"}，不要其它内容。"
            )
